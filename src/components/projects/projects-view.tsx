...
const NEUTRAL_GRADIENT = "linear-gradient(135deg, #64748B, #94A3B8)";

/** Colored header treatment per column — Trello-style, not grey-on-grey. */
const STATUS_COLUMN_STYLE: Record<string, { bar: string; wash: string; text: string }> = {
...
function highestPriority(issues: { priority: string }[]): string | null {
  ...
  return topRank > 1 ? top : null; // only surface medium+ on the card
}
